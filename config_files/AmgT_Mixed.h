...
extern "C" {
#endif
    extern double time_spgemm;
    extern double time_spgemm_preprocess;
    extern int spgemm_times;
    extern int rap;
    extern int dia;
    extern char *matrix_name;
    extern double cusparse_spgemm_time;

    extern double csr2bsr_step1;
    extern double csr2bsr_step2;
    extern double csr2bsr_step3;
    extern double bsr2csr_step1;
    extern double bsr2csr_step2;
    extern double bsr2csr_step3;
    /******************************************************************************
...
// #define PRINT_KERNEL_PERFORMANCE
...
#define getbit(x, y) ((x) >> (y) & 1) // get the yth bit of x
    ...
    typedef struct
    {
        ...
        MAT_VAL_TYPE *blcVal;
        MAT_MAP_TYPE *blcMap;
        MAT_PTR_TYPE warpnum;       // load-balanced value
        MAT_PTR_TYPE *rowPtrbyWarp; // load-balanced array
        MAT_IDX_TYPE *rowIdxbyWarp; // load-balanced array
...
#define Bitmap_col_x_row(col, row) (((col) * 0xf) & ((row) * 0x1111))
    ...
    typedef struct
    {
        // AMGT
        int coasen_level;
        char bsr_spmv_balanced_tag;
        char bsr_tag;
        char vector_space_flag;
        char mixed_precision_flag;
        char tag_mid_matrix;
        bsrMAT *bsr_mat;
 ...
 *--------------------------------------------------------------------------*/
#define hypre_BSRBALANCEDTAG(matrix) ((matrix)->bsr_spmv_balanced_tag)
#define hypre_BSRTAG(matrix) ((matrix)->bsr_tag)
#define hypre_BSR(matrix) ((matrix)->bsr_mat)
#define hypre_MidMatrixTag(matrix) ((matrix)->tag_mid_matrix)
#define hypre_VectorSpaceTag(matrix) ((matrix)->vector_space_flag)
#define hypre_MixedPrecisionTag(matrix) ((matrix)->mixed_precision_flag)
#define hypre_Level(matrix) ((matrix)->coasen_level)
...
HYPRE_Int hypre_CSRMatrixSpMVDevice( HYPRE_Int trans, HYPRE_Complex alpha, hypre_CSRMatrix *A,
    ...
    __global__ void CSR_TO_BSR_getidx_noshare(int blc_row, int blc_col, int csr_row, int csr_col, int every_layer,
    ...
    // void CSR_TO_BSR_step2(int bsr_row, int bsr_col, int csr_row, int csr_col, MAT_PTR_TYPE *blcPtr,
    //                       MAT_IDX_TYPE *blcIdx, MAT_VAL_TYPE *blcVal, int *blcMap,
    //                       MAT_PTR_TYPE *csrPtr, MAT_IDX_TYPE *csrIdx, MAT_VAL_TYPE *csrVal, int *sum_tmp_memory);
    double CSR_TO_BSR_step2(int bsr_row, int bsr_col, int csr_row, int csr_col, MAT_PTR_TYPE *blcPtr,
                            ...
                            MAT_PTR_TYPE *csrPtr, MAT_IDX_TYPE *csrIdx, MAT_VAL_TYPE *csrVal);
    __global__ void bsr_spmv_balanced(int *rowPtrbyWarp, int *rowIdxbyWarp, int warp_num,
