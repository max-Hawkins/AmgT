...
int constexpr size_of_vector { 1000000 };
...
class nvmlClass {
  public:
    ...
    void getStats( ) {
        ...
        stats device_stats {};
        // Create NVML field values struct
        // int num_fields = 2;
        // nvmlFieldValue_t *field_values = (nvmlFieldValue_t *)malloc( sizeof( nvmlFieldValue_t ) * num_fields );
        // // NVML_FI_POWER_USAGE_INSTANT
        // // NVML_FI_POWER_USAGE_AVERAGE
        // // pynvml.NVML_FI_DEV_POWER_AVERAGE, pynvml.NVML_POWER_SCOPE_MEMORY
        // // https://ml.energy/zeus/reference/device/gpu/nvidia/
        // field_values[0].fieldID = NVML_FI_DEV_POWER_INSTANT;
        // field_values[0].scopeID = 0; // GPU only

        // field_values[1].fieldID = NVML_FI_DEV_POWER_AVERAGE;
        // field_values[1].scopeID = 0; // GPU only
// nvmlReturn_t nvmlDeviceGetFieldValues ( nvmlDevice_t device, int  valuesCount, nvmlFieldValue_t* values )
        ...
        while ( loop_ ) {
            ...
            // NVML_RT_CALL( nvmlDeviceGetFieldValues( device_, num_fields, field_values ) );
            // // Check if the field values are valid
            // if( field_values[0].nvmlReturn != NVML_SUCCESS ) {
            //     printf( "Invalid field ID: %d\n", field_values[0].fieldID );
            //     // device_stats.powerUsageInstant = 0;
            // }else{
            //     printf( "Value type (i: %d): %d\n", 0, field_values[0].valueType );
            //     // device_stats.powerUsageInstant = field_values[0].value;
            // }
