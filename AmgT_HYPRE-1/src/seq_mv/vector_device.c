 ...
 *--------------------------------------------------------------------------*/
__global__ void dotProductKernel(MAT_VAL_TYPE *a, MAT_VAL_TYPE *b, MAT_VAL_TYPE *result, int N)
{
   // 获取全局线程的ID
   int idx = threadIdx.x + blockDim.x * blockIdx.x;

   // 用于临时存放每个线程计算的部分内积结果的共享内存
   ...
   // 同步block内的所有线程，确保每个线程都已完成自己的乘法操作
   __syncthreads();
   ...
   while (i != 0)
   {
      ...
      // 同步block内的所有线程，确保每次归约步骤都完成
      __syncthreads();
      i /= 2;
   }

   // 最后将计算出的和放到结果数组的对应位置上
   if (threadIdx.x == 0)
   {
                                ...
                                hypre_Vector *y )
{
   ...
   struct timeval t_Compute1, t_Compute2;
   double time_dot = 0;

   // printf("num_vectors:%d %d\n", total_size, num_vectors);

#if defined(HYPRE_USING_GPU)

#if defined(HYPRE_USING_CUDA) || defined(HYPRE_USING_HIP)
#if defined(HYPRE_USING_CUBLAS)
   gettimeofday(&t_Compute1, NULL);
   dim3 blockSize(256); // 假设有256个线程每个block
   ...
   // // int threadNum = 256;
   // // int blockNum = (total_size + threadNum - 1) / threadNum;
   dotProductKernel<<<gridSize, blockSize>>>(x_data, y_data, result_gpu, total_size);

   cudaMemcpy(&result, result_gpu, sizeof(HYPRE_Real), cudaMemcpyDeviceToHost);
   // HYPRE_CUBLAS_CALL(hypre_cublas_dot(hypre_HandleCublasHandle(hypre_handle()), total_size,
   //                                    x_data, 1, y_data, 1, result_gpu));
   // cudaMemcpy(&result, result_gpu, sizeof(HYPRE_Real), cudaMemcpyDeviceToHost);
   cudaDeviceSynchronize();
   gettimeofday(&t_Compute2, NULL);
   time_dot += (t_Compute2.tv_sec - t_Compute1.tv_sec) * 1000.0 + (t_Compute2.tv_usec - t_Compute1.tv_usec) / 1000.0;
   // printf("device dot time=%lf\n", time_dot);
#else
