...
double time_interp = 0;
double time_coarsen = 0;
double time_smooth = 0;
double time_max_level = 0;
double time_initial = 0;
double time_RAP = 0;
double time_corsen_type = 0;
double time_other = 0;
                      ...
                      hypre_ParVector    *u )
{
   ...
   while (not_finished_coarsening)
   {
      ...
      time_initial += (t10.tv_sec - t9.tv_sec) * 1000.0 + (t10.tv_usec - t9.tv_usec) / 1000.0;
      if (max_levels == 1)
      {
         S = NULL;
      ...
      else /* max_levels > 1 */
      {
         ...
         if (nodal == 0) /* no nodal coarsening */
         {
            ...
            time_corsen_type += (te.tv_sec - ts.tv_sec) * 1000.0 + (te.tv_usec - ts.tv_usec) / 1000.0;
         }
         else if (block_mode)
         {
            if (coarsen_type == 6)
         ...
         else /* no aggressive coarsening */
         {
            ...
            {
               /*插值算法*/
               // printf("hypre_BoomerAMGBuildExtPIInterp\n");
               ...
               time_interp += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
            }
      ...
      else if (mult_addlvl == -1 || level < mult_addlvl || level > add_end)
      {
         ...
         else
         {
            ...
            {
               // printf("RAPKT\n");
               hypre_ParCSRMatrixLevel(P_array[level]) = level;
               hypre_ParCSRMatrixLevel(A_array[level]) = level;
               struct timeval t1, t2;
               gettimeofday(&t1, NULL);
               A_H = hypre_ParCSRMatrixRAPKT(P_array[level], A_array[level],
                                             P_array[level], keepTranspose);
               gettimeofday(&t2, NULL);
               time_RAP += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
            }
            else
            {
   ...
   time_coarsen += (t4.tv_sec - t3.tv_sec) * 1000.0 + (t4.tv_usec - t3.tv_usec) / 1000.0;
   // printf("setup Coarsening end\n"); // zlj 添加
   // printf("time_corsen_type=%.5lf\n",time_corsen_type);
   // printf("time_interp=%.5f\n",time_interp);
   // printf("time_RAPKT=%.5lf\n",time_RAP);
   ...
   // time_smooth += (t6.tv_sec - t5.tv_sec) * 1000.0 + (t6.tv_usec - t5.tv_usec) / 1000.0;
   // printf("time_smooth=%.5lf\n",time_smooth);
   // printf("time_max_level=%.5lf\n",time_max_level);
   ...
   time_other += (t_e.tv_sec - t_s.tv_sec) * 1000.0 + (t_e.tv_usec - t_s.tv_usec) / 1000.0;
   // printf("time_other=%.5f\n", time_other);
