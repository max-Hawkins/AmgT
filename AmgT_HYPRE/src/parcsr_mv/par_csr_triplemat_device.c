...
{
      // printf("hypre_ParCSRMatMatDevice\n");
   ...
   hypre_ParPrintf(comm, "Time hypre_ParCSRMatMatDevice %f\n", tb);
      ...
      time_spgemm_all += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
...
hypre_ParCSRMatrixRAPKTDevice( hypre_ParCSRMatrix *R,
                               ...
                               HYPRE_Int           keep_transpose )
{
      // printf("hypre_ParCSRMatrixRAPKTDevice\n");
      struct timeval t1, t2;
      gettimeofday(&t1, NULL);
      MPI_Comm comm = hypre_ParCSRMatrixComm(A);
   ...
   else
   {
      ...
      hypre_Level(R_diagT) = hypre_ParCSRMatrixLevel(R);
      hypre_Level(A_diag) = hypre_ParCSRMatrixLevel(A);
      hypre_Level(P_diag) = hypre_ParCSRMatrixLevel(P);
      // printf("RAP\n");
