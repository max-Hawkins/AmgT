                                   ...
                                   hypre_ParVector *y)
{
   ...
   if (exec == HYPRE_EXEC_DEVICE)
   {
      ...
      time_spmv += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
      // printf("time_spmv=%f", time_spmv);
   }
   else
#endif
   {
                            ...
                            HYPRE_Int fpt)
{
   ...
   if (num_procs > 1)
   {
      ...
      num_sends = hypre_ParCSRCommPkgNumSends(comm_pkg);
      if (num_sends)
         x_buf_data = hypre_CTAlloc(HYPRE_Complex, hypre_ParCSRCommPkgSendMapStart(comm_pkg, num_sends), HYPRE_MEMORY_HOST);
      ...
      for (i = 0; i < num_sends; i++)
      {
         ...
         for (j = start; j < hypre_ParCSRCommPkgSendMapStart(comm_pkg, i + 1); j++)
            x_buf_data[index++] = x_local_data[hypre_ParCSRCommPkgSendMapElmt(comm_pkg, j)];
      }
      comm_handle =
   ...
   if (num_procs > 1)
   {
      ...
      if (num_cols_offd)
         hypre_CSRMatrixMatvec_FF(alpha, offd, x_tmp, 1.0, y_local,
