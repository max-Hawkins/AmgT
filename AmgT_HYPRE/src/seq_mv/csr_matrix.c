                       ...
                       HYPRE_Int num_nonzeros )
{
   ...
   // AMGT
   // AMGT
   hypre_BSRTAG(matrix) = 0;
   hypre_BSR(matrix) = NULL;
   hypre_BSRBALANCEDTAG(matrix) = 0;
   hypre_Level(matrix) = 0;
   hypre_VectorSpaceTag(matrix) = 0;
   hypre_MixedPrecisionTag(matrix) = 0;
...
hypre_CSRMatrixDestroy( hypre_CSRMatrix *matrix )
{
   if (matrix)
   {
      ...
      if (hypre_BSRTAG(matrix) == 1)
      {

         bsrMAT *bsr_mat = hypre_BSR(matrix);
         cudaFree(bsr_mat->blcPtr);
         cudaFree(bsr_mat->blcIdx);
         cudaFree(bsr_mat->blcVal);
         cudaFree(bsr_mat->blcMap);
         free(bsr_mat);
      }
